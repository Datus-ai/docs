...
- Copy and check example dataset (california_schools)  
- Check success_story.csv  
- Check reference_sql/ directory  
- Update agent.yml with these 
...
- **workflow**: Defines the execution flow for each agent. These [workflows](../workflow/introduction.md) are designed to output results to files, making it easy to evaluate and compare agent performance.
  - Step 1: Use subagent above to analyzes the question and generates SQL
  - Step 2: `execute_sql` node executes the generated SQL to generate final result
  - Step 3: `output` node formats and returns the results to local disk
...
```
or use chatbot in [Datus-Chat](../web_chatbot/introduction.md) 


## 3. Compare Subagent Performance

This is the key part of the tutorial:  
Comparing a **non-context** agent vs. a **context-rich** agent.
...
## 5. Multi-round Benchmark: Demonstrate Context Evolution
