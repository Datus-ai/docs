...
Use `datus/multi_round_benchmark.py` to execute repeated benchmark + evaluation cycles and compare the outcomes.
...
- `--workflow`: The name of the workflow used by the benchmark.
- `--max_round`: number of benchmark/evaluation loops (default 4).
- `--task_ids`: explicit task id list (space separated).
...
- `save/` – The directory where the SQL and results are located.
- `trajectory/` – The trajectory process of the workflow.
- `evaluation_round_{round}.json` – the evaluation report.
