...
4. 使用 `benchmark/california_schools/reference_sql`的sql文件构建参考SQL
...
使用 `datus/multi_round_benchmark.py` 来执行重复的基准+评估循环并比较结果。
...
- `--workflow`: 基准测试使用的工作流名称。
- `--max_round`: 基准测试/评估循环次数（默认4）。
- `--task_ids`: 要执行任务ID列表（空格分隔），默认执行全部任务。
- `--max_workers`: 与常规基准标志用于控制并发相同。

对于每一轮，脚本都会将代理的输出目录重写为“{agent.home}/integration/{workflow}_{round}/'，因此以下目录将保持隔离：

- `save/` – Datus生成SQL和结果所在的目录。
- `trajectory/` – 工作流的执行过程。
- `evaluation_round_{round}.json` – 始评估报告

所有轮次结束后，每个任务的评估结果会汇总为'{agent.home}/integration/{workflow}_summary.xlsx' ，格式为：
...
1. `Not Executed`: 基准或评估遇到异常.
2. `Matched`: 与标准答案成功匹配
3. `Gen SQL Failed`:代理生成的SQL在数据库中无法执行
4. `Gold SQL Failed`: 数据库中的金色SQL执行失败了。检查配置是否错误，或者Gold SQL 查询是否有缺陷。
5. `Match Failed`: 评估结果未能获得，可能是因为未被发现的异常。
6. `Table Mismatch`: 表示使用的数据表不准确（错误或缺失）。
7. `Result Mismatch`: 表匹配成功，但行或列匹配失败。
8. `Column Mismatch`: 表、行和某些列成功匹配